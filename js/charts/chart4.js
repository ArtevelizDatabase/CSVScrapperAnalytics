// Chart 4: Unique Products by User (Horizontal Bar Chart)
const UniqueProductsChart = {
  instance: null,
  create(canvasId, data, options = {}) {
    ...
    const defaultOptions = {
      ...
      scales: {
        x: { beginAtZero: true, title: { display: true, text: "Unique Products", font: { size: 11 } }, grid: { color: "rgba(0,0,0,0.05)" }, ticks: { font: { size: 10 }, callback: (value) => value.toLocaleString() } },
        y: { title: { display: true, text: "User / Author", font: { size: 11 } }, grid: { display: false }, ticks: { font: { size: 10 } } },
      },
    };
    const mergedOptions = { ...defaultOptions, ...options };
    try {
      this.instance = new window.Chart(ctx.getContext("2d"), { type: "bar", data, options: mergedOptions });
      console.log("UniqueProductsChart created successfully via chart4.js");
      return this.instance;
    } catch (e) { console.error("Error creating UniqueProductsChart:", e); return null; }
  },
...
window.UniqueProductsChart = UniqueProductsChart;
console.log("chart4.js loaded: UniqueProductsChart is now global.");